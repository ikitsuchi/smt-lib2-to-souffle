...
type sort_def =
  { sort_name : string
  ; sort_params : string list
  ; sort_gen : string
  ; sort_gen_args : string list
  }
...
and stmt =
  ...
  | Stmt_def_sort of sort_def
  | Stmt_decl_rel of string * string list
  | Stmt_decl_var of ty_var * ty
...
let def_sort ?loc s ~params ~gen ~gen_args =
  let d =
    { sort_name = s; sort_params = params; sort_gen = gen; sort_gen_args = gen_args }
  in
  _mk ?loc (Stmt_def_sort d)
;;

let decl_rel ?loc s l = _mk ?loc (Stmt_decl_rel (s, l))
let decl_var ?loc s ty = _mk ?loc (Stmt_decl_var (s, ty))
...
let data_zip ?loc decls cstors =
  if List.length decls <> List.length cstors
  then
    parse_errorf
      ?loc
...
let rule ?loc t = _mk ?loc @@ Stmt_rule t
...
let rec pp_ty out (ty : ty) =
  ...
  | Ty_arrow (args, ret) -> fpf out "(@[=>@ %a@ %a@])" (pp_list pp_ty) args pp_ty ret
;;
...
let rec pp_term lvl out (t : term) =
  ...
  let fpf' lvl' out fmt =
    if lvl <> lvl' then fpf out "(@[" else fpf out "(";
    Format.kfprintf
  ...
  | Arith (op, l) -> Format.fprintf out "(@[<hv>%a@ %a@])" pp_arith op (pp_list self_a) l
  ...
  | Match (lhs, cases) ->
    let pp_case out = function
      ...
      | Match_case (c, [], rhs) -> fpf out "(@[<1>%s@ %a@])" c (self' _lvl_top) rhs
      | Match_case (c, vars, rhs) ->
        fpf out "(@[<1>(@[%s@ %a@])@ %a@])" c (pp_list pp_str) vars (self' _lvl_top) rhs
    in
    fpf'
      lvl_match
      out
  ...
  | Fun (v, body) -> fpf' lvl_q out "lambda @ (%a)@ %a" pp_typed_var v (self' lvl_q) body
  | Let (l, t) ->
  ...
  | Imply (a, b) -> fpf' lvl_or out "=>@ %a@ %a" (self' lvl_or) a (self' lvl_or) b
...
let pp_prop_lit out (s, b) = if b then fpf out "%s" s else fpf out "(not %s)" s
...
let pp_stmt out (st:statement) = match view st with
   ...
   | Stmt_query s -> fpf out "(@[query@ %s@])" s
   | Stmt_rule t -> fpf out "(@[rule@ %a@])" pp_term t
   | Stmt_def_sort s -> fpf out "(@[define-sort@ %s@ (%a@ %a@ %a)@])" s.sort_name (pp_list pp_str) s.sort_params pp_str s.sort_gen (pp_list pp_tyvar) s.sort_gen_args
   | Stmt_decl_rel (s,l) -> fpf out "(@[declare-rel@ %s@ (%a)@])" s (pp_list pp_str) l
   | Stmt_decl_var (s,ty) -> fpf out "(@[declare-var@ %s@ %a@])" s pp_ty ty
